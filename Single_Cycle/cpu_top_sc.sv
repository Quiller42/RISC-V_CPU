module cpu_top_sc (
    ...
    pc_adder_sc pc_adder (
        .pc_in(current_pc),
        .pc_plus_4(pc_plus_4)
    );

    // Instantiate instruction memory
    instruction_memory_sc instr_mem (
        .pc(current_pc),
        .instruction(instruction)
    );

    // Instantiate register file
    register_file_sc reg_file (
        .clk(clk),
        .rst_n(rst_n),
